...
*weeding efficiency (percentage herbicide savings compared to baseline)

set technoAttr / areaCapac, eff, fieldTime, varMachCostsPestiRedFactor /;

*data entry of technoloy parameters
parameter p_technoData(technoAttr,pestType) /
    "areaCapac".set.pestType 7200
    "eff"."herb" 50
*    "eff"."fung" 50
    "varMachCostsPestiRedFactor"."herb" 1
...
e_deprecTechno(years)..
    ...
    $ (curPlots_ktblSize(curPlots,KTBL_size) AND curPlots_ktblDistance(curPlots,KTBL_distance) AND curPlots_ktblYield (curPlots,KTBL_yield) AND ktblCrops_ktblSystem(curCrops,KTBL_system) AND p_pestOpFreq(curCrops,KTBL_system,KTBL_yield,'herb')),
...
*step wise reduction of technology area capacity
    p_technoData("areaCapac",pestType) = 7500 - 1000 * (areaCapacStep.pos - 1);
        ...
        p_cropCapac(curCrops,KTBL_system,KTBL_yield,pestType) 
            $ (ktblCrops_ktblSystem(curCrops,KTBL_system) AND p_pestOpFreq(curCrops,KTBL_system,KTBL_yield,pestType) AND p_technoData("eff",pestType))
